...
onMounted(async () => {
  try {
    const [active, extended] = await Promise.all([
      contractService.getAll({ page: 1, pageSize: 100, status: 1 }),
      contractService.getAll({ page: 1, pageSize: 100, status: 3 })
    ])
    contracts.value = (active.data?.data?.items || []).concat(extended.data?.data?.items || [])
  } catch (error) {
...
<template>
  <div>
    <h1 class="text-2xl font-bold text-white mb-6">Danh Sách Chuộc Hàng</h1>
